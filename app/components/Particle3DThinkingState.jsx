...
export default function Particle3DThinkingState({
  size = 80,
  color = '#006fcf',
  running = true,
  speedMultiplier = 1,
  ...
  const r = size * 0.35;
  const particles = useMemo(() => generateParticles(180, r), [r]);
  ...
  useEffect(() => {
    ...
    const tick = (now) => {
      ...
      containerRef.current.style.setProperty('--particle-rot', `${baseRotation}deg`);
    };
  ...
  return (
    <div style={{ position: 'relative', width: size, height: size, display: 'inline-flex', perspective: '1200px' }} ref={containerRef}>
      ...
      {/* Center Anchor Point with 3D perspective */}
      <div style={{
        ...
        {/* 3D particles with individual rotation */}
        {particles.map((p, i) => (
          <div
            key={i}
            style={{
              ...
              background: color,
              opacity: 0.8,
              transform: `translate3d(calc(${p.sx}px * (1 - var(--morph-t, 0)) + ${p.cx}px * var(--morph-t, 0)), calc(${p.sy}px * (1 - var(--morph-t, 0)) + ${p.cy}px * var(--morph-t, 0)), calc(${p.sz}px * (1 - var(--morph-t, 0)) + ${p.cz}px * var(--morph-t, 0))) rotateX(calc(var(--particle-rot, 0deg) * ${(i % 3) === 0 ? 1 : (i % 3) === 1 ? -0.7 : 0.5})) rotateY(calc(var(--particle-rot, 0deg) * ${(i % 5) === 0 ? 1.2 : (i % 5) === 1 ? -0.8 : 0.6})) rotateZ(calc(var(--particle-rot, 0deg) * ${(i % 7) === 0 ? 0.9 : (i % 7) === 1 ? -1.1 : 0.4}))`
            }}
          />
        ))}
