...
import InputBarsPage from './InputBarsPage';
...
export default function ComparisonPage() {
  const [currentPage, setCurrentPage] = useState('thinking-states');
  ...
  if (currentPage === 'input-bars') {
    return <InputBarsPage onPageChange={setCurrentPage} />;
  }
  ...
  return (
    <div className="flex flex-col items-center min-h-screen py-16 px-8 bg-background">
      ...
      <Tabs defaultValue="explorations" className="w-full max-w-6xl">
        <TabsList className="grid w-full grid-cols-2 mb-8">
          <TabsTrigger value="explorations">Explorations</TabsTrigger>
          <TabsTrigger value="discarded">Discarded Ideas</TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="explorations">
          <div className="grid w-full grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            ...
            {/* Cell 4 */}
            <Card label="4. CSS Cross Rings (Glass)" glowFilter={glowFilter} componentName="AnimatedSphere">
              <AnimatedSphere size={80} color={colorHex} logo={<AmexLogo size={40} />} durations={currentDurations} useWhiteOutline />
            </Card>

            {/* Cell 5 */}
            <Card label="5. CSS Cross Rings" glowFilter={glowFilter} componentName="AnimatedSphere">
              <AnimatedSphere size={80} color={colorHex} logo={<AmexLogo size={40} />} durations={currentDurations} />
            </Card>

            {/* Cell 8 */}
            <Card label="8. CSS GPU Particle Swarm" glowFilter={glowFilter} componentName="ParticleThinkingState">
              <ParticleThinkingState size={80} color={colorHex} logo={<AmexLogo size={40} />} durations={currentDurations} />
            </Card>

            {/* Cell 11 */}
            <Card label="11. SVG Masking Reveal" glowFilter={glowFilter} componentName="ClipPathThinkingState">
              <ClipPathThinkingState size={80} color={colorHex} logo={<AmexLogo size={40} />} durations={currentDurations} />
            </Card>

            {/* Cell 14 */}
            <Card label="14. CSS Orthogonal Rings" glowFilter={glowFilter} componentName="OrthogonalRingsThinkingState">
              <OrthogonalRingsThinkingState size={80} color={colorHex} logo={<AmexLogo size={40} />} durations={currentDurations} />
            </Card>

            {/* Cell 15 */}
            <Card label="15. GPU Particle Dots Only" glowFilter={glowFilter} componentName="DotParticleThinkingState">
              <DotParticleThinkingState size={80} color={colorHex} logo={<AmexLogo size={40} />} durations={currentDurations} />
            </Card>

            {/* Cell 16 */}
            <Card label="16. 3D Sphere Particles" glowFilter={glowFilter} componentName="Particle3DThinkingState">
              <Particle3DThinkingState size={80} color={colorHex} logo={<AmexLogo size={40} />} durations={currentDurations} />
            </Card>

            {/* Cell 17 */}
            <Card label="17. ASCII Geometric" glowFilter={glowFilter} componentName="AsciiGeometricThinkingState">
              <AsciiGeometricThinkingState size={80} color={colorHex} durations={currentDurations} />
            </Card>

            {/* Cell 18 */}
            <Card label="18. ASCII Wireframe Globe" glowFilter={glowFilter} componentName="AsciiWireframeThinkingState">
        ...
        <TabsContent value="discarded">
          <div className="grid w-full grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            ...
            {/* Cell 1 */}
            <Card label="1. 2D SVG Bezier Math" glowFilter={glowFilter} componentName="ThinkingState">
              <ThinkingState size={80} color={colorHex} logo={<AmexLogo size={40} />} durations={currentDurations} onPhase={setPhase} />
            </Card>

            {/* Cell 2 */}
            <Card label="2. Pure CSS 3D Box" glowFilter={glowFilter} componentName="Css3DThinkingState">
              <Css3DThinkingState size={80} color={colorHex} logo={<AmexLogo size={40} />} durations={currentDurations} />
            </Card>

            {/* Cell 3 */}
            <Card label="3. Framer + Flubber Extrusion" glowFilter={glowFilter} componentName="FramerThinkingState">
              <FramerThinkingState size={80} color={colorHex} logo={<AmexLogo size={40} />} durations={currentDurations} />
            </Card>

            {/* Cell 6 */}
            <Card label="6. HTML5 Canvas Paint" glowFilter={glowFilter} componentName="CanvasThinkingState">
              <CanvasThinkingState size={80} color={colorHex} logo={<AmexLogo size={40} />} durations={currentDurations} />
            </Card>

            {/* Cell 7 */}
            <Card label="7. CSS Origami Net Fold" glowFilter={glowFilter} componentName="OrigamiThinkingState">
              <OrigamiThinkingState size={80} color={colorHex} logo={<AmexLogo size={40} />} durations={currentDurations} />
            </Card>

            {/* Cell 10 */}
            <Card label="10. Soft-Body Elastic Physics" glowFilter={glowFilter} componentName="SoftBodyThinkingState">
              <SoftBodyThinkingState size={80} color={colorHex} logo={<AmexLogo size={40} />} durations={currentDurations} />
            </Card>

            {/* Cell 13 */}
            <Card label="13. Solid 3D Math Mesh" glowFilter={glowFilter} componentName="SolidMathThinkingState">
              <SolidMathThinkingState size={80} color={colorHex} logo={<AmexLogo size={40} />} durations={currentDurations} />
            </Card>

            {/* Cell 9 */}
            <Card label="9. ASCII Typographic Matrix" glowFilter={glowFilter} componentName="AsciiThinkingState">
              <AsciiThinkingState size={80} color={colorHex} durations={currentDurations} />
            </Card>

            {/* Cell 12 */}
            <Card label="12. 3D Math Wireframe" glowFilter={glowFilter} componentName="WireframeThinkingState">
...
function Card({ label, children, glowFilter, componentName }) {
  return (
    <CardWithQuickView label={label} glowFilter={glowFilter} componentName={componentName}>{children}</CardWithQuickView>
  );
}
