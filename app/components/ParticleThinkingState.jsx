...
function generateParticles(numParticles, r) {
  ...
  for(let i = 0; i < numParticles; i++) {
    ...
    const u = (prB * 2 - 1) * r;
    const v = (prC * 2 - 1) * r;
    if (face===0) { cx=u; cy=v; cz=r; }   // Front
    if (face===1) { cx=u; cy=v; cz=-r; }  // Back
    if (face===2) { cx=u; cy=r; cz=v; }   // Bottom
    if (face===3) { cx=u; cy=-r; cz=v; }  // Top
    if (face===4) { cx=r; cy=u; cz=v; }   // Right
    if (face===5) { cx=-r; cy=u; cz=v; }  // Left
